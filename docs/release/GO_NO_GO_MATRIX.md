...
| Chocolatey validation artifacts | Manual | No | Run the `Publish Chocolatey` workflow with `publish=false` after the GitHub release exists |
| Chocolatey publication evidence | Manual | No | Run `Publish Chocolatey` with `publish=true` only after prior versions clear Chocolatey moderation |
...
- GO for GitHub/winget release does not require Chocolatey evidence because Chocolatey is manually gated by moderation state.
